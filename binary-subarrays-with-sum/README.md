...
This is a clever approach that uses a sliding window to solve the problem in linear time. The core idea is to transform the problem from finding subarrays with a sum *exactly* equal to `goal` to finding subarrays with a sum *at most* `goal`.
